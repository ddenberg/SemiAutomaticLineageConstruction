function P = ConstrainedMinimization_Penalty(X0, A1, A2, B1, B2, lambda, division_constraints, lineage_constraints, penalty)
...
options = optimoptions('fmincon', 'Algorithm', 'interior-point', 'Display', 'iter-detailed', ...
    'SpecifyObjectiveGradient', true, 'SubproblemAlgorithm', 'cg', 'EnableFeasibilityMode', true);

tic;
p_vec = fmincon(@(X) objective_penalty(X, A1, A2, B1, B2, K1, K2, lambda, A, b, penalty), X0(:), ...
...
function [f, grad] = objective_penalty(x_vec, A1, A2, B1, B2, K1, K2, lambda, A, b, penalty)
N1 = size(A1, 1);
N2 = size(A2, 1);
...
f = 0.5 * trace(D1.' * D1) + 0.5 * lambda * trace(D2.' * D2) + penalty * g;

grad = (X * A2 * (X.' * X) * A2.' + X * A2.' * (X.' * X) * A2) - (A1.' * X * A2 + A1 * X * A2.') + ...
       lambda * (0.5 * X * (K2.' + K2) - K1);

grad = grad(:);
grad = grad + penalty * (dc_dx * dg_dc);
end
...
function H = hessian_function(x_vec, A1, A2, B1, B2, K1, K2, lambda)
N1 = size(A1, 1);
N2 = size(A2, 1);
X = reshape(x_vec, N1, N2);
...
H_tensor = zeros(N1, N2, N1, N2);
for ii = 1:N1
    for jj = 1:N2
        dX_dX_iijj = zeros(N1, N2);
        dX_dX_iijj(ii,jj) = 1;
        H_tensor(:,:,ii,jj) = 2 * (dX_dX_iijj * temp_mat1 + temp_mat2 * (dX_dX_iijj.' * X + X.' * dX_dX_iijj) * A2.') - ...
            2 * (A1 * dX_dX_iijj * A2) + lambda * dX_dX_iijj * temp_mat3;
    end
end

H = reshape(H_tensor, N1*N2, N1*N2);
